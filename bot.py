...
def message_input(message):
    ...
    if message.text == 'Первый':
        ...
        bot.send_message(message.from_user.id,
                         text='Бот не работает с текстовыми сообщениями, выберите интересующий экспонат')
...
def handle_docs_audio(message):
    ...
    with open(
            f'input_sound/{message.from_user.first_name}_{message.from_user.last_name}_@{message.from_user.username}_{message.voice.file_id}.ogg',
            'wb') as new_file:
